...
{
  const inputStudyDate = document.getElementById('js-studyDate')
  const calendar = document.getElementById('js-calendar')

  /**
   * 開く
   */
  inputStudyDate.addEventListener('click', () => {
    calendar.setAttribute('aria-hidden', 'false')
  })

  // js-decideCalendarというidをもつDOMを取得し、clickイベントを監視する
  document.getElementById('js-decideCalendar').addEventListener('click', ()=>{
    // カレンダーモーダルを閉じる
  ...
  inputStudyDate.value = studyDateText()
  ...
  function createCalendar(){
    ...
    const tbody = document.getElementById('js-calendarBody');
    ...
    document.getElementById('js-calendarTitle').textContent = calendarThisMonth;
    ...
    weeks.forEach(week => {
      ...
      week.forEach(date => {
        ...
        // tdに日付を設定する
        td.innerHTML = `<span>${date.date}</span>`;
  ...
  function addSetToday() {
    ...
    const validaDates = document.querySelectorAll('#js-calendarBody td:not(.--disabled)')
    // 取得した日にちでループを回す
    validaDates.forEach(td => {
      // tdには日にちのDOM1つが入り、それがclickされた時の挙動を設定する
      td.addEventListener('click', (e) => {
        // 学習日を更新する
        inputStudyDate.value = studyDateText(e.target.textContent)
        // selected-dateというclassをもつtdを取得する
        const selectedDate = document.querySelector('td.--selected')
  ...
  document.getElementById('js-calendarPrev').addEventListener('click', ()=>{
    // 月を一つ前にする
    calendarMonth--;
  ...
  document.getElementById('js-calendarNext').addEventListener('click', ()=> {
    // 月を一つ後にする
    calendarMonth++;
